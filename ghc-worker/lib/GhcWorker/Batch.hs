...
import Types.Log (TraceId (..), newLog)
import Types.State (newStateWith)
...
batchCompileWith tmp env extraOptions paths = do
  ...
  let
    dispatch' opts bargs =
      dispatch lock WorkerMakeMode hooksNoop (withOptions opts) bargs \ _ -> pure (FeatureInstrument False)

  createDirectoryIfMissing True out
  _ <- dispatch' (metadataOptions ++ extraOptions ++ paths) buckArgs {BuckArgs.mode = Just ModeMetadata}
  for_ paths \ path -> do
    let name = takeFileName path
        hash = out </> addExtension name "hash"
    dbg name
    dispatch' (compileOptions name ++ [path]) buckArgs {BuckArgs.mode = Just ModeCompile, BuckArgs.abiOut = Just hash}
  where
    out = tmp </> "out"
...
batchCompile argv =
  ...
  log <- newLogger <$> newLog (Just (TraceId "batch"))
