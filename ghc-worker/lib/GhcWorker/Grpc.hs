...
triggerTask _stateVar _recompile = \case
  TaskTrigger {task = Metadata} ->
    pure (Failure "Cannot trigger metadata")
  TaskTrigger {task = Build _rebuild} ->
    pure (Failure "Cannot trigger build")
  TaskTrigger {task = Execute} ->
    pure (Failure "Cannot trigger execute")
...
apiRequest chan stateVar recompile = \case
  ...
  EvictBytecode req ->
    Ack <$ evictBytecode stateVar chan req
  Clean _ ->
    pure (Failure "Cleaning not supported")
