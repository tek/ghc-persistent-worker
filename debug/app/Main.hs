...
import GHC.Debug.Retainers (findRetainersIncremental)
...
findThunks roots =
  findRetainers (Just 100000) flt roots
  where
...
sanitizeStack ::
  ...
  [(SizedClosureP, SourceInformation)]
sanitizeStack enable
  | enable = \case
    [] -> []
    (scp, (Just si)) : t -> (scp, si) : mapMaybe sanitize t
    _ : t -> sanitizeStack enable t
  | otherwise = mapMaybe sequence

displayGrouped :: Bool -> Maybe Int -> Grouped -> IO ()
displayGrouped sanitized limit =
  traverse_ display . fmap (filter nonNullStack . fmap (fmap (sanitizeStack sanitized)) . toList)
  where
    display stacks = do
...
addLoc c =
  (maybe "unknown" (show . fst) (uncons c),) <$> addLocationToStack c
...
thunks =
  ...
  where
    prep roots = do
      rs <- findThunks roots
      rs' <- addLocs rs
      pure (dropWhile (\ a -> length a <= minCount) (sortOn length (groupAllWith byName rs')))

    process = displayGrouped True (Just 5)
...
strings =
  ...
  where
    ...
    target :: String
    target = "buck-out/v2/gen/toolchains/904931f735703749-8d78dcf3e0e36516/__haskell__/__action___0__/dep43/out.link/lib/ghc-9.10.1/lib/package.conf.d"
...
new :: IO ()
new =
  snapshotRun "test" \ e -> do
    pause e
    run e do
      _ <- precacheBlocks
      roots <- gcRoots
      findRetainersIncremental Nothing (ConstructorDescFilter ((== ":") . name)) roots \ r -> do
        r' <- addLoc r
        unsafeLiftIO $ displayStack [sanitizeStack True <$> r']

main :: IO ()
main =
  snapshotRun "snapshots/lib444-ospath" debug
