...
import Ghc.Ui.Server.Process (defaultSocketPath, isServerUp)
import Ghc.Ui.Server.Start (serverConnect, serverHandlers)
import Ghc.Ui.Server.Stop (cleanAndStop)
...
runBrick :: Options -> ServerM ()
runBrick options = do
  server <- serverHandlers options
  (_, vty) <- finally (run server) do
...
main = do
  ...
  options <- liftIO $ execParser optionsInfo
  flip runReaderT ServerEnv {executable = options.serverExe, events, process} do
    runClock
    maybe (withoutSpecified workers) withSpecified specifiedSocket (runBrick options)
