...
import Brick.Types (EventM, vpTop)
...
completeTask :: Int -> Outcome -> EventM Name TasksState ()
...
phaseStart requestId phase =
  modifying listElementsL $ fmap \case
    TaskRow task | task.requestId == requestId ->
      TaskRow task {
        phase = Just phase,
        phases = Map.insertWith (\ _new old -> old) phase (PhaseInfo {order = length task.phases, durationMs = 0}) task.phases
      }
    row -> row
