...
classifyProject ProjectBuild {initial = InitialProject {unitCount, moduleCount}, incrementalMetadata} = do
  label "── project size ──"
  ...
  classify "  incremental metadata" incrementalMetadata
  classify "  full metadata" (not incrementalMetadata)
