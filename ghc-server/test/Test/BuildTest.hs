...
test_uiRebuildRace =
  uiReproTest "repeated UI rebuild ('b', 'r', 'r') does not corrupt an intra-unit dependency" \ tp -> do
    ...
    chan <- liftIO newChan
    liftIO $ triggerTask (Just chan) cb tp.project TaskTrigger {target = Instrument.TargetProject, task = Instrument.Build False}
    liftIO $ awaitGenerationSettled _decisions 1
    liftIO $ triggerTask (Just chan) cb tp.project TaskTrigger {target = Instrument.TargetProject, task = Instrument.Build True}
...
-- 'UI.Project.selectedCompileTargets') without waiting for any of them to complete before
