-- | gRPC request handler for the GHC server.
...
import GhcServer.Data.Unit (ClientModule (..), Project (..), Unit (..), UnitCache (..), UnitName (..), fromUiUnitName)
import GhcServer.Log (emitLog, newLogger)
import GhcServer.Path (cacheDirName, outputDirName, tmpDirName)
...
import Test.Scheduler (Phase (..), SchedulerResources (..), SchedulerState (..))
import qualified Text.Parsec as Parsec
import qualified Types.Args as Args
import Types.Args (emptyArgs)
import Types.FeatureFlags (FeatureFlags (..))
import Types.Grpc (CommandEnv (..), RequestArgs (..))
import Types.Instrument (HomeModule (..), Target (..), toGhcModuleName)
...
-- | Intermediate type for extracting flags from schedule args.
data Flags =
  Flags {
    wait :: Bool,
    recompile :: Bool,
...
-- - @unitName:modules@ compile all modules
-- - @unitName:ModuleName@ compile a specific module
--
...
handle :: Project -> Build -> RequestArgs -> IO ([String], Int32)
handle project build (RequestArgs argv) =
  either parseError run (parseScheduleArgs project argv)
  where
    run cmd = do
      scheduleBatch build cmd.request
      if cmd.scheduleWait
      then wait
      else pure (["Scheduled."], 0)

    wait =
      awaitBuild build <&> \ result ->
        (formatResult result, if result.success then 0 else 1)

    parseError message = pure ([message], 1)
...
serverContext config = do
  ...
  projectCabal <- join <$> whenMaybe (not config.jsonConfig) do
  ...
  env <- buildEnv config outputDir tmpDir project log
  ...
  build <- newBuild (\ decision -> emitLog env.instrChan "scheduler" "debug" (show decision)) config.maxJobs 300 env
  pure ServerContext {
    grpcHandler = GrpcHandler (const (handle project build)),
    build,
    buildEnv = env
  }
  where
...
parseCleanTarget :: Text -> CleanTarget
...
--
-- Dispatched through the unified 'Types.Instrument.Command' (the @Clean@ constructor) rather than its own gRPC
-- method, since @ghc-server.proto@'s @GhcServer@ service was merged down to just @Execute@\/@Events@\/@Api@ (see
-- 'GhcServer.Grpc.runCommand'). Returns a success flag and human-readable message instead of building a proto
-- response directly, since that's now 'GhcServer.Grpc''s job (JSON-encoding a 'Types.Instrument.Response').
--
...
runClean :: Build -> BuildEnv -> Target -> IO (Bool, Text)
runClean build env = \case
  TargetProject -> do
  ...
  TargetUnit {name = uiName} | let name = fromUiUnitName uiName ->
    case Map.lookup name env.project.units of
      Nothing -> failure ("Unknown unit: " <> Text.pack name.string)
      Just unit -> do
        removed <- filterM removeIfExists [unit.cache.dir, env.outputDir </> toOsPath name.string]
        invalidateUnitState build name
        modifyMVar_ env.diff (pure . Map.delete name)
        success (describeRemoved removed)
  TargetModule {key = HomeModule {unit = uiUnit, name = uiName}} -> do
    let unit = fromUiUnitName uiUnit
        name = toGhcModuleName uiName
    case Map.lookup unit env.project.units of
      Nothing -> failure ("Unknown unit: " <> Text.pack unit.string)
      Just _ -> do
        invalidateModuleState build unit name
        success "Invalidated scheduler state for module (no on-disk cache removed)."
  where
    success msg = pure (True, msg)
    failure msg = pure (False, msg)

    describeRemoved removed
      | null removed = "Nothing to clean."
      | otherwise = "Removed: " <> Text.intercalate ", " (Text.pack . fromOsPath <$> removed)
