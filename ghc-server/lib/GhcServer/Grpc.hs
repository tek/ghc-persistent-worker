-- | gRPC 'GhcServer' service for the standalone GHC server (@ghc-server.proto@). Serves all three RPCs the
-- protocol now exposes: 'Execute' (delegates to 'GhcServer.Handler.executeCommand'), 'Events' (server-streaming
-- instrumentation feed, replacing the former separate @Instrument@ service's @NotifyMe@), and 'Api' (JSON-payload
-- command\/response, replacing @Instrument@'s unified @Send@ RPC). Merged into a single service so 'ghc-server'
-- only ever listens on one Unix socket instead of two.
...
import GhcServer.Data.Unit (ClientModule (..), Project (..), Unit (..), UnitName (..), fromUiUnitName, toUiUnitName)
...
import Proto.GhcServer (GhcServer, Json)
...
projectStructureEvent project =
  ProjectStructure {
    units = [UnitSummary {name = toUiUnitName name, modules = [ModuleName (Text.pack (moduleNameString m)) | (m, _) <- unit.modules]} | (name, unit) <- Map.toList project.units]
  }
...
targetToUnitRequest project = \cases
  ...
  TargetUnit {name} kind ->
    [(fromUiUnitName name, unitRequest kind)]
  TargetModule {key = HomeModule {unit, name}} kind ->
    [(fromUiUnitName unit, moduleRequest name kind)]
  where
    unitRequest = \case
    ...
    moduleRequest name = \case
      Metadata -> UnitMetadata
      TaskKind.Build {} -> UnitModules [ClientModule (Text.unpack (coerce name))]
      Execute -> UnitExecute
...
runCommand mchan build env project = \case
  TriggerTask trigger -> triggerTask mchan build project trigger $> Ack
  EvictBytecode req -> do
    Worker.applyEviction env.stateVar req
    for_ mchan (Worker.pushBytecodeState env.stateVar)
    pure Ack
  Clean target -> uncurry CleanResult <$> runClean build env target
...
-- pinned to @ghc-server.proto@'s @Json@ message, used for both request and response.
handleApi ::
...
handleApi run req = do
  resp <- case eitherDecodeStrict req.payload of
    Left err -> do
      IO.hPutStrLn IO.stderr ("handleApi: failed to decode payload: " ++ err)
      pure Ack
    Right cmd -> run cmd
  pure (defMessage & Fields.payload .~ LBS.toStrict (Aeson.encode resp))
