...
import GhcServer.Data.Unit (UnitName (..), toUiUnitName)
...
import Types.Instrument (Event (..), HomeModule (..), Target (..), fromGhcModuleName)
...
dispatchTask env ext task = case task.key of
  MetaTask name
    | task.value.runMeta -> do
      ...
      withTaskEvents env requestId TargetUnit {name = toUiUnitName name} (taskResultFromErrors . fst <$> runMetadata env name)
    | otherwise -> skipMetadata env name
  ResolvedModule unit name -> do
    let target = TargetModule {key = HomeModule {unit = toUiUnitName unit, name = fromGhcModuleName name}}
    requestId <- nextRequestId env
    withTaskEvents env requestId target (compile ext env unit name requestId)
  ExecuteModule unit name -> do
    let target = TargetModule {key = HomeModule {unit = toUiUnitName unit, name = fromGhcModuleName name}}
    requestId <- nextRequestId env
...
computeResolutions cache env name _state =
  cache.loadUnit name >>= \case
    Nothing -> throwE ("computeResolutions: missing cached_unit.json for " <> Text.pack name.string <> " after successful metadata")
    Just cu -> do
      liftIO $ logEvent env.events (ResolutionComputed name)
...
propagateCompletion cache env (MetaTask name) (TaskSuccess _) state =
  runExceptT (computeResolutions cache env name state) >>= \case
    Left err -> do
      let errS = Text.unpack err
      env.log.debug ("Cache decode failure during propagation: " ++ errS)
      pure state {failures = Map.insert (MetaTask name) errS state.failures}
    Right newModules -> do
