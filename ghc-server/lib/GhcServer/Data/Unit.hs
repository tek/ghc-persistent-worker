...
import qualified Types.Instrument as Instrument
...
fromUiUnitName :: Instrument.UnitName -> UnitName
fromUiUnitName name = UnitName (Text.unpack name.text)

toUiUnitName :: UnitName -> Instrument.UnitName
toUiUnitName UnitName {string} = Instrument.UnitName (Text.pack string)
...
data Unit =
  Unit {
    ...
    ghcArgs :: [String],
    -- | The unit's modules, each paired with the absolute path of its source file.
    modules :: [(ModuleName, OsPath)],
...
mkUnitCache projectRoot name =
  UnitCache {
    dir = cDir,
    cachedUnitPath = cDir </> [osp|cached_unit.json|],
    unitArgsPath = cDir </> [osp|unit_args|],
    depUnitsPath = cDir </> [osp|dep_units.json|],
    sourceDigestsPath = cDir </> [osp|source_digests.json|],
    sourceHashesPath = cDir </> [osp|source_hashes.json|]
  }
  where
    cDir = projectRoot </> cacheDirName </> toOsPath name.string
