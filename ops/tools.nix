...
in {
  ...
  config = {
    ...
    # Create the action metadata JSON with digests for all source files
    ...
    echo "Starting ghc-server with ACTION_METADATA..."
    ...
    if (( ''${#features[@]} > 0 )); then
      ...
      use_fixed_nodes=false
      for f in "''${features[@]}"; do
        case $f in
          incremental-metadata) use_incremental=true ;;
          fast-flag-parser) use_fast_parser=true ;;
          fixed-nodes) use_fixed_nodes=true ;;
          *) echo "Unknown feature: $f"; exit 1 ;;
        esac
      done
    fi
    flag_args=(--fixed-nodes $use_fixed_nodes --fast-flag-parser $use_fast_parser --incremental-metadata $use_incremental)

    echo "Features: fixed-nodes=$use_fixed_nodes fast-flag-parser=$use_fast_parser incremental-metadata=$use_incremental"

    # Compute ACTION_METADATA JSON for unitbig sources.
    metadata_file=$project/action_metadata.json
    compute_metadata() {
      local meta='{"version": 1, "digests": ['
      local first=true
    ...
    origdir=$PWD
    cd $project
    ${profiledFixedPkg}/bin/ghc-server ''${flag_args[@]} $project +RTS -p -RTS &
    server_pid=$!
    cd $origdir
    ...
    {
      "features": {
        "fixed_nodes": $use_fixed_nodes,
        "fast_flag_parser": $use_fast_parser,
        "incremental_metadata": $use_incremental
      },
      "project": {
        "depth": $depth,
...
ENDJSON
    ...
    cp $result_json stats.json
    if [[ -f $prof_file ]]; then
      cp $prof_file ghc-server.prof
      ${profiteur}/bin/profiteur $prof_file
      cp ''${prof_file%.prof}.prof.html ghc-server.prof.html
    fi
    '';

    outputs.apps.compare-features = util.zapp "compare-features" ''
    depth=''${1-8}
    big_mods=''${2-50}
    ...
    nix run .#profile-production -- $depth $big_mods fixed-nodes fast-flag-parser incremental-metadata 2>&1
    cp stats.json /tmp/compare-all-stats.json 2>/dev/null || true

    echo ""
    echo "--- Run 2: Without incremental-metadata ---"
    nix run .#profile-production -- $depth $big_mods fixed-nodes fast-flag-parser 2>&1
    cp stats.json /tmp/compare-no-incr-stats.json 2>/dev/null || true

    s1=/tmp/compare-all-stats.json
    s2=/tmp/compare-no-incr-stats.json
