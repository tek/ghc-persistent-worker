{
  ...
  outputs = {hix, ghc-debug, ...}: let
    ...
    envOverrides = config: {overrideAttrs, nodoc, ...}: {
  ...
  in hix ({config, build, lib, util, ...}: let
    ...
    defaultEnv = extra: args: {
      env = testEnv args.config;
      hls.enable = lib.mkForce false;
      package-set.extends = "mwb-26-01";
      overrides = commonOverrides ["mwb" "unit-index" "downsweep-cache"] args ++ [ipeOverrides] ++ extra;
    };
  ...
  in {
    ...
    ghcVersions = [];
    main = "ghc-worker";
    ghci.args = ["-package ghc" "-DMWB" "-DUNIT_INDEX" "-DDOWNSWEEP_CACHE"];
    hls.genCabal = false;
    ...
    envs.mwb-26-01 = args: defaultEnv [buckBinOverrides] args // {
      expose.scoped = true;
    };
    ...
    envs.profiled = args: {
      env = testEnv args.config;
      package-set.extends = "mwb-26-01";
      overrides = commonOverrides ["unit-index" "downsweep-cache"] args ++ [ipeOverrides];
    };
