{
  ...
  in hix ({config, build, lib, ...}: let

    localOptions.options = {

      oldMwbGhcBranch = lib.mkOption {
        description = "Whether to use the GHC branch with the old versions of the bytecode patches.";
        type = lib.types.bool;
        default = false;
      };
  ...
  in {
    ...
    packages = let
      ghc-options = mwbCppOpts ++ ["-DMWB" "-O2"];
    in {
