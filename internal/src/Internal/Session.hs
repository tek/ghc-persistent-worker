...
setTempDir dir = updateGlobalFlags \ dflags -> dflags {tmpDir = TempDir dir}
  ...
  withGhc \ env srcs run -> do
    ...
    withState env.log env.state do
      ...
      modifySessionM \ hsc_env0 -> do
        ...
        hsc_env2 <- processArg hsc_env1 (loadHomeUnit env.log env.state dflags0 env.args.featureFlags (moduleUnitId target.mod)) env.args.homeUnit
