...
-- | Append a @-D@ or @-U@ preprocessor flag, mirroring 'addOptP' from @GHC.Driver.Session@.
--
-- Updates both 'toolSettings_opt_P' and its fingerprint as required by GHC.
addCppFlag :: String -> DynFlags -> DynFlags
addCppFlag f dflags =
  let s = toolSettings dflags
      newOpts = f : toolSettings_opt_P s
  in dflags {toolSettings = s {
...
-- | Parse @-D@ CPP definition flags (@-DFOO@ or @-DFOO=BAR@).
--
-- Mirrors GHC's @AnySuffix (upd . addOptP)@ for the @-D@ flag:
-- the full flag string (e.g. @"-DFOO=BAR"@) is passed to 'addOptP'.
prefixDFlag :: Parser () Flag
prefixDFlag = do
  byteString "-D"
  rest <- byteStringOf takeRest
  pure (FlagNoArg (addCppFlag (B8.unpack ("-D" <> rest))))

-- | Parse @-U@ CPP undefinition flags (@-UFOO@).
--
-- Mirrors GHC's @AnySuffix (upd . addOptP)@ for the @-U@ flag.
prefixUFlag :: Parser () Flag
prefixUFlag = do
  byteString "-U"
  rest <- byteStringOf takeRest
  pure (FlagNoArg (addCppFlag (B8.unpack ("-U" <> rest))))
