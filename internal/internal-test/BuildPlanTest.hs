  ...
  BuildPlanSchema {
    ...
    cache = Just [
      ("U1M1", CachedModule {
        source = "U1M1.hs",
        modules = [],
        packages = [CachedPackageDep {id = jui "base", modules = [jmn "Prelude"]}]
      }),
      ("U1M2", CachedModule {
        source = "U1M2.hs",
        modules = [jmn "U1M1"],
  ...
  BuildPlanSchema {
    ...
    cache = Just [
      ...
      ("U2M2", CachedModule {
        source = "Dummy.hs",
        modules = [jmn "U2M1"],
        packages = if oneshot then reverse cachedPDeps else cachedPDeps
      })
    ]
  }
  where
    cachedPDeps =
      [
        CachedPackageDep {id = jui "unit1", modules = [jmn "U1M2"]},
        CachedPackageDep {id = jui "base", modules = [jmn "Prelude"]}
      ]
